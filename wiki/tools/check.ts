...
const BANNED_LEGACY_STRINGS = ["_standards", "MASTERFILE_STANDARD", "WIKI_CSS_STYLE_RESEARCH"];
...
for (const bannedString of BANNED_LEGACY_STRINGS) {
  if (wikiCss.includes(bannedString)) {
    errors.push(`_shared/wiki.css: references retired "${bannedString}" — the standards live in .claude/skills/wiki/ now`);
  }
}
